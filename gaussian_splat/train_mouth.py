...
from .motion_net import MouthMotionNetwork
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    with progress:
        for iteration in range(first_iter, opt.iterations + 1):
            ...
            loss_l1 = Ll1
            loss_dssim = opt.lambda_dssim * (1.0 - ssim(image_green, gt_image_green))
            loss_alpha = torch.zeros([], device=gt_image.device)
            loss_lpips_patch = torch.zeros([], device=gt_image.device)
            loss = loss_l1 + loss_dssim

            if iteration > warm_step:
                loss_alpha = 1e-3 * (((1-alpha) * lips_mask).mean() + (alpha * ~lips_mask).mean())
            ...
            with torch.no_grad():
                ...
                if iteration % 10 == 0:
                    ...
                    progress.update(
                        task_id,
                        status=f"Loss={ema_loss_for_log:.5f} AU25={au_lb:.1f}-{au_ub:.1f}",
                    )
